...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 z-50 flex h-[65px] max-h-[65px] w-full min-w-full max-w-full items-center justify-between bg-[#03001417] px-4 shadow-lg shadow-[#2A0E61]/50 backdrop-blur-md md:px-10 ">
      <div
        className=" mx-auto flex w-full min-w-full  max-w-full items-center justify-between p-4"
        onClick={() => {
          setSelectedNav("Home");
        }}
      >
        <Link href="/">
          <div className="logo__font whitespace-nowrap  text-3xl font-bold text-yellowcolor">
            Rashika Suresh
          </div>
        </Link>
        ...
        >
          <div className="flex">
            <ul className="mr-10 mt-4 flex flex-col rounded-lg border border-gray-700  p-4  font-medium md:mt-0 md:flex-row md:space-x-8 md:border-0  md:p-0  rtl:space-x-reverse">
              {navItems &&
                navItems?.length > 0 &&
          ...
          >
            <Link href={"/contact"}>
              ...
              />
              <div
                className={`group relative whitespace-nowrap rounded-full bg-black px-8 py-2 text-white transition duration-200 hover:bg-transparent ${
                  selectedNav === "Contact"
